...
class Manager{
    ...
    public function updateCruise($data){
        ...
        if (empty($data->image)) {
            ...
            $this->db->query('UPDATE `cruise` SET `name`= :name ,`departure_date`= :date,`nights_number`=:nbr,`price`=:price,`departure_port_ID`=:idd WHERE `ID`=:id');
            ...
            $this->db->query('UPDATE `cruise` SET `name`= :name ,`departure_date`= :date,`nights_number`=:nbr,`Price`=:price,`departure_port_ID`=:idd,`image`=:img WHERE `ID`=:id');
