...
<div class="relative overflow-x-hidden shadow-md sm:rounded-lg w-1/2 mx-auto">
    <table class="w-full text-sm text-left ">
        ...
        <form action="<?php echo URLROOT . 'Pages/book/' . $data['cruise'][0]->ID_croisere . '/' . $data['cruise'][0]->ID_user . '/' . $data['cruise'][0]->price; ?>" method="POST">
        <tbody>
                <?php foreach ($data['rooms'] as $room) : ?>
                    <tr class="bg-white border-b">
                        ...
                        <td class="px-6 py-4">
                            <input type="radio" name="room" value="<?= $room->ID_room; ?>"></inout>
                        </td>
                    </tr>
                <?php endforeach; ?>
