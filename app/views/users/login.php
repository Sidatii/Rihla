...
<div class="w-full my-5 max-w-sm p-4 bg-white border border-gray-200 rounded-lg shadow-md sm:p-6 md:p-8 mx-auto">
    <form class="space-y-6" action="<?php echo URLROOT; ?>/users/login" method="POST">
        ...
        <div>
            ...
            <input type="email" name="email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 <?php echo (!empty($data['email_err'])) ? 'border-red-500' : ''; ?>" placeholder="name@company.com" value="<?php echo $data['email']; ?>">
